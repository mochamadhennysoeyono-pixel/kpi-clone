...
export default function KpiDataPage() {
  const { kpiData, companies, deleteKpiData, employees } = useMasterData();
  ...
  const handleDelete = async () => { if (dataToDelete) { await deleteKpiData(dataToDelete.map(d => d.id)); setDataToDelete(null); } };

  const showCompanyFilter = userRole === 'superadmin' || isHoldingAdmin;

  return (
    <ResponsivePage>
      <PageHeader title="Arsip Data KPI" description="Pusat penyimpanan permanen seluruh catatan capaian kinerja karyawan." icon={Database} />
      <ResponsiveToolbar>
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
          <Input placeholder="Cari nama karyawan..." className="pl-9 h-10 border-none shadow-none bg-background/50" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      ...
      <AdaptiveTable 
        ...
        keyExtractor={(d) => d.id}
        columns={[
          { header: "Karyawan", cell: (d) => (
              <div className="flex items-center gap-3">
                <div className="size-9 rounded-xl bg-primary/5 text-primary flex items-center justify-center border shrink-0"><User size={18} /></div>
                <div className="min-w-0"><p className="font-bold text-slate-900 truncate">{d.employeeName}</p><p className="text-[10px] text-muted-foreground uppercase font-black truncate">{d.position}</p></div>
              </div>
          )},
          ...
          { header: "Status", cell: (d) => <Badge variant={d.status === 'Melampaui Target' ? 'default' : 'secondary'} className="text-[9px] font-black uppercase h-5 border-none">{d.status}</Badge> },
          { header: "Persetujuan", cell: (d) => <Badge variant={d.approvalStatus === 'Disetujui' ? 'outline' : 'destructive'} className="text-[8px] font-bold gap-1.5 border-none bg-muted/50">{d.approvalStatus}</Badge> },
          ...
          <Card className="border-border/40 shadow-sm overflow-hidden" onClick={() => setSelectedKpiData(d)}>
            <CardContent className="p-4 space-y-4">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                    <div className="size-10 rounded-xl bg-primary/5 text-primary flex items-center justify-center border shrink-0"><User size={20} /></div>
              ...
              <div className="flex items-center justify-between pt-3 border-t">
                  ...
                  <div className="flex items-center gap-1.5 text-[9px] font-bold text-muted-foreground uppercase">PRATINJAU <ArrowRight size={10} /></div>
              </div>
            </CardContent>
          </Card>
        )}
      ...
      <DeleteConfirmationDialog isOpen={isDeleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirm={handleDelete} itemName={dataToDelete?.length === 1 ? `data ${dataToDelete[0].employeeName}` : `${dataToDelete?.length} data`} itemType="pencapaian KPI" />
    </ResponsivePage>
  );
}
