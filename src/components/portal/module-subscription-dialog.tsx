...
// Map light bg to dark bg for header contrast
...
export function ModuleSubscriptionDialog({ 
    ...
    const rawBg = module.bg.split(' ')[0].replace('/20', '');
    const headerBg = headerBgMap[rawBg] || 'bg-slate-900';

    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none shadow-2xl rounded-3xl flex flex-col max-h-[90vh]">
                <div className={cn("p-6 md:p-8 text-white relative shrink-0", headerBg)}>
                    <div className="absolute top-0 right-0 p-4 opacity-10">
                        <Icon size={100} />
                    </div>
                    <DialogHeader className="relative z-10 text-left">
                        <div className="p-2.5 bg-white/20 backdrop-blur-md rounded-xl w-fit mb-4">
                            <Icon size={24} className="text-white" />
                        </div>
                        <DialogTitle className="text-xl md:text-2xl font-black uppercase tracking-tight">Kustomisasi {module.name}</DialogTitle>
                        <DialogDescription className="text-white/80 font-medium text-xs md:text-sm">
                ...
                <DialogFooter className="p-6 shrink-0 flex flex-col gap-3 bg-background border-t">
                    <Button 
                        onClick={() => handleAction('paid')} 
                        disabled={isLoading}
                        className="h-11 w-full font-black uppercase tracking-widest shadow-xl transition-all active:scale-95"
                    >
                    ...
                    {!hasUsedTrial ? (
                        <Button 
                            ...
                            disabled={isLoading}
                            className="h-11 w-full font-bold border-2 text-primary hover:bg-primary/5 rounded-xl transition-all"
                        >
                            <Zap size={18} className="mr-2" /> Coba Gratis 14 Hari
                        </Button>
                    ) : (
                        <p className="text-center text-[9px] text-muted-foreground font-medium italic opacity-60">
