...
const ModuleLogo = ({ name }: { name: string }) => (
...
const UserMenu = ({ user, role, onLogout }: { user: any, role: string, onLogout: () => void }) => (
  <DropdownMenu>
    <DropdownMenuTrigger asChild>
      <Button variant="ghost" className="h-10 gap-3 px-2 rounded-xl hover:bg-muted shrink-0">
        <div className="hidden sm:flex flex-col items-end">
          <span className="text-xs font-black uppercase tracking-tight">{user?.name?.split(' ')[0]}</span>
          <span className="text-[9px] font-bold text-muted-foreground uppercase opacity-60 leading-none">{role}</span>
        </div>
    ...
    <DropdownMenuContent align="end" className="w-56 rounded-xl shadow-2xl border-none p-2">
      ...
      <DropdownMenuItem onClick={onLogout} className="p-3 rounded-lg cursor-pointer text-destructive focus:bg-destructive/10 focus:text-destructive">
        <LogOut size={14} />
...
export function ModuleHeader() {
  ...
  const activeModule = useMemo(() => getActiveModuleFromPath(pathname), [pathname]);
  ...
  return (
    <header className="h-14 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 flex items-center px-4 sticky top-0 z-[100] shadow-sm no-print w-full">
      <div className="flex items-center gap-4 w-full max-w-7xl mx-auto">
        ...
        <ModuleLogo name={currentModuleName} />
